...
// from 为源对象，返回目标类型对象及 error
// 可通过 Option 自定义 tagName 等行为
// 若已注册自定义转换器则优先使用，否则使用默认反射拷贝
// from 为 nil 时，返回 DIST 的零值
func Convert[SRC, DIST any](from SRC, opts ...Option) (DIST, error) {
	var zero DIST
	if isNil(any(from)) {
		return zero, nil
	} // 处理 Option
	...
	mu.RUnlock()
	if ok {
		return fn.(Converter[SRC, DIST])(from)
	}

	return defaultConvert[SRC, DIST](from, opt.tagName)
}
